...
export default function HeroSection({onSearch, selectedColor, onColorChange, selectedOrientation, onOrientationChange, selectedTopic, onTopicChange,logo}) {
  ...
  return (<section className="container mx-auto px-4 py-8 relative">
      <div className="flex flex-col md:flex-row gap-8">
        <div className="md:w-1/2">
          ...
          <div className="max-w-md space-y-4">
            ...
            {/* Ориентация */}
            <OrientationFilters selectedOrientation={selectedOrientation} onChange={onOrientationChange}
                                disabled={!query.trim()}/>
            {/* Темы */}
            <TopicsFilters selectedTopic={selectedTopic} onChange={onTopicChange} disabled={!query.trim()}/>
          </div>
        </div>

        <div className="md:w-1/2">
          <div className="grid grid-cols-2 gap-2">
            {collections.map(c => (<NavLink
                key={c.id}
                to={`/collections/${c.id}`}
